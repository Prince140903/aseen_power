...
export default function ClientsView() {
  ...
  return (
    <section className="py-24 bg-white dark:bg-[#1a1c22] border-b border-[#e9e8e7] dark:border-[#3a3d45] relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6">
        {/* Section Header */}
        <ScrollReveal variant="fadeUp" className="text-center mb-20">
          ...
          <p className="font-sans text-sm text-[#666766] dark:text-[#8b8e93] mt-4 max-w-2xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="overflow-hidden rounded-lg">
            ...
            >
              {clients.map((client, index) => (
                ...
                >
                  <div className="w-full h-full bg-[#fbf9f8] dark:bg-[#23252d] border border-[#e9e8e7] dark:border-[#3a3d45] rounded flex items-center justify-center p-3 sm:p-4 hover:border-[#785919]/30 dark:hover:border-[#eac076]/30 hover:shadow-md transition-all duration-300 cursor-pointer group">
                    <div className="relative w-full h-full flex items-center justify-center">
                      <Image
                        ...
                        width={200}
                        height={200}
                        className="object-contain max-w-[85%] max-h-[70px] filter grayscale group-hover:grayscale-0 transition-all duration-300 dark:brightness-90 dark:group-hover:brightness-100"
                        onError={(e) => {
                          const img = e.target as HTMLImageElement;
                          img.style.display = 'none';
                          const placeholder = document.createElement('div');
                          placeholder.textContent = client.name;
                          placeholder.className = 'font-display font-semibold text-[9px] sm:text-xs text-center text-[#666766] dark:text-[#8b8e93] px-2';
          ...
          <button
            ...
            className="absolute -left-6 sm:-left-9 top-1/2 transform -translate-y-1/2 z-10 p-1.5 sm:p-2 rounded-full bg-white dark:bg-[#1a1c22] border border-[#e9e8e7] dark:border-[#3a3d45] text-[#785919] dark:text-[#eac076] hover:bg-[#785919] dark:hover:bg-[#eac076] hover:text-white dark:hover:text-black transition-all duration-300 shadow-md hidden sm:flex items-center justify-center cursor-pointer group"
          ...
          <button
            ...
            onMouseEnter={() => setIsAutoPlay(false)}
            onMouseLeave={() => setTimeout(() => setIsAutoPlay(true), 2000)}
            className="absolute -right-9 top-2/5 transform -translate-y-1/2 z-10 p-2 rounded-full bg-white dark:bg-[#1a1c22] border border-[#e9e8e7] dark:border-[#3a3d45] text-[#785919] dark:text-[#eac076] hover:bg-[#785919] dark:hover:bg-[#eac076] hover:text-white dark:hover:text-black transition-all duration-300 shadow-md hidden md:flex items-center justify-center cursor-pointer group"
            ...
            {Array.from({ length: Math.ceil(clients.length / 5) }).map((_, index) => (
              <motion.button
                ...
                className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 cursor-pointer ${Math.floor(currentIndex / 5) === index
                    ? 'bg-[#785919] dark:bg-[#eac076] w-6 sm:w-8'
                    : 'bg-[#e9e8e7] dark:bg-[#3a3d45] hover:bg-[#c4c7c7] dark:hover:bg-[#8b8e93] w-1.5 sm:w-2'
                  }`}
        ...
        {/* Stats Section */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20 pt-16 border-t border-[#e9e8e7] dark:border-[#3a3d45]">
          <motion.div
          ...
          >
            ...
            <p className="font-sans text-sm text-[#666766] dark:text-[#8b8e93]">Projects Completed</p>
          </motion.div>
          ...
          >
            ...
            <p className="font-sans text-sm text-[#666766] dark:text-[#8b8e93]">Industry Leaders</p>
          </motion.div>
          ...
          >
            ...
            <p className="font-sans text-sm text-[#666766] dark:text-[#8b8e93]">Years of Excellence</p>
          </motion.div>
        </div>
      </div>
    </section>
