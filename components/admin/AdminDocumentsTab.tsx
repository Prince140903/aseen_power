  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="font-display font-bold text-2xl text-black uppercase tracking-tight mb-1">Protected Documents</h2>
          <p className="text-sm text-gray-600">Manage certifications, licenses, and credentials</p>
        </div>
        {!isAdding && !editingId && (
          <button
            onClick={() => {
        ...
        >
          ...
          <div className="mb-6">
            ...
            {editingId && formData.file_url && !selectedFile && (
              <p className="text-xs text-gray-500 mt-2">Current file: {formData.file_url.split('/').pop()}</p>
            )}
          </div>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-display font-bold text-base text-black mb-2">{document.title}</h3>
                  <p className="text-sm text-gray-600 mb-3">{document.description}</p>
                  <p className="text-xs text-gray-500">PDF stored securely in Supabase Storage</p>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                <span className="text-xs text-gray-400">Order: {document.order}</span>
                {document.file_url && (
                  <a
                    href={document.file_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-[#785919] hover:text-black font-display font-bold uppercase"
                  >
                    Download PDF →
                  </a>
                )}
